...
export const adminAuthResolvers = {
    Query: {
        me: async (_: any, __: any, context: AdminContext) => {
            ...
            const adminData = await AdminAuthService.getAdminWithPermissions(context.admin.adminId)
        ...
        verifyAdminToken: async (_: any, { token }: { token: string }, context: AdminContext) => {
            try {
    ...
    Mutation: {
        adminLogin: async (_: any, { input }: { input: any }) => {
            ...
            try {
                ...
                const isValidPassword = await AdminAuthService.verifyPassword(password, admin.password || "")
                if (!isValidPassword) {
                    throw new GraphQLError("Invalid credentials", {
                        extensions: { code: "INVALID_CREDENTIALS" },
                    })
                ...
                // Get admin roles
                const adminData = await AdminAuthService.getAdminWithPermissions(admin.id)
                const roles = adminData?.roles.map((role) => role.roleSlug) || []

                const sessionId = AdminAuthService.createAdminSession(admin.id, admin.email, roles)
        ...
        verifyAdminOTP: async (_: any, { input }: { input: any }) => {
            ...
            try {
                ...
                const otpRecord = await AdminAuthService.verifyAdminOTP(admin.id, otp, type)
        ...
        adminLogout: async (_: any, { token }: { token: string }, context: AdminContext) => {
            try {
                if (token) {
        ...
        refreshToken: async (_: any, { input }: { input: any }) => {
            try {
                ...
                const session = await AdminAuthService.validateRefreshToken(refreshToken)
                if (!session) {
                    return {
                        success: false,
                        token: null,
                ...
                // Create new tokens
                const newAccessToken = createSession(admin.id, admin.email, roles.join(","))
